...
HOOK="${ROOT}/plugins/knowledge-distillery/install/hooks/pre-prompt-knowledge-gate.sh"
...
assert_contains "${GATE_SKILL}" '<skill-directory>/scripts/knowledge-gate' "knowledge-gate skill should resolve the CLI from its own install directory"
assert_contains "${GATE_SKILL}" 'hook-provided exact path' "knowledge-gate skill should prefer the concrete path supplied by the prompt hook"
assert_not_contains "${GATE_SKILL}" 'CLAUDE_PLUGIN_ROOT' "knowledge-gate skill should not require a Claude plugin environment"
assert_contains "${HOOK}" '"${CLI_PATH}" get <id> or "${CLI_PATH}" get-many <id...>' "pre-prompt hook should mention detail fetch with the resolved CLI path"
assert_contains "${INSTALL_GUIDE}" 'Fetch full details only for needed IDs with `knowledge-gate get <id>` or `get-many <id...>`' "agent installation guide should add summary-first retrieval guidance"
assert_contains "${INSTALL_GUIDE}" '## Decision Recording' "agent installation guide should add decision-recording guidance"
