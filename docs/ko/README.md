...
| 커맨드 | 용도 |
|---|---|
| `query-paths [--ids-only] <filepath>` | 파일 경로를 도메인으로 해소 → 경량 summary index 또는 ID만 반환 |
| `query-domain [--ids-only] <domain>` | 도메인 이름으로 규칙 조회 → 경량 summary index 또는 ID만 반환 |
| `search [--ids-only] <keyword>` | FTS5 전문 키워드 검색 → 경량 summary index 또는 ID만 반환 |
| `get <id>` | 항목 전체 상세 조회 (body 포함) |
| `get-many <id>...` | 여러 항목의 전체 상세를 한 번에 조회 |
| `list [--ids-only]` | 활성 항목 요약 목록 또는 ID만 반환 |
...
# 도메인 규칙을 초경량 ID 인덱스로만 조회
knowledge-gate query-domain --ids-only payment

# 경량 조회 뒤 필요한 전체 항목만 배치로 가져오기
