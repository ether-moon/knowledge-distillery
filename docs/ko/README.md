...
파이프라인 동작의 source of truth는 `skills/`에 있는 실행 가능한 Skill들이다.
...
| `domain-list [--status X]` | 도메인 목록 (active\|deprecated\|all) |
...
플러그인은 표준 Claude Code 레이아웃을 따른다: 메타데이터는 `.claude-plugin/plugin.json`에 두고, 실제 자산은 plugin root의 `skills/`, `scripts/`, `schema/`에 둔다.
