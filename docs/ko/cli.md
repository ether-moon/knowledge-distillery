...
| 패턴 미커버 | 최근 배치 PR diff 파일 중 20%+가 도메인 미해소 | 패턴 추가 필요 (`batch-refine` 리포트에서 표면화, CLI는 미담당) |
...
**구현 참고:** 현재 CLI 구현은 vault/repo 상태만으로 계산 가능한 도메인 밀도와 구조 불일치를 진단한다. 최근 batch PR diff 맥락이 필요한 패턴 미커버 평가는 `domain-report`가 아니라 `batch-refine` 리포트 단계에서 수행한다.
