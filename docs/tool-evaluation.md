...
- Adoption validation is handled by `/knowledge-distillery:setup`, which both sets up and verifies repository configuration. Future diagnostics may add `knowledge-gate vault-health` and related read-only status commands, but these must remain **auxiliary interfaces that do not grant vault write permissions**.
