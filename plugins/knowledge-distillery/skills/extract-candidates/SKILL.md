...
9. If a new domain is proposed, is the name self-explanatory enough to be recognized from the ID alone in `domain-list --ids-only`?
10. If a new domain is proposed, does its `description` define the scope clearly enough to distinguish it from adjacent domains?
11. Before proposing a new domain, did the skill check whether an existing domain could be reused instead?
12. If a reused or proposed domain revealed a naming/scope problem, was that follow-up captured in `_domain_maintenance` rather than left implicit?
