...
1. Reuse the existing-entry set already fetched by `extract-candidates` Step 2 for these domains — it is in-memory in this **same subagent context**, so do NOT re-issue `query-domain` per domain. **Fallback:** if that set is unavailable (e.g., extract-candidates hit the `query-domain` error path and proceeded with null `conflict_check`), fetch it now so the duplicate check is never silently skipped:
   ```bash
...
R7 is the **sole authoritative derivability verifier** with file-reading capability. extract-candidates criterion 4d is only a cheap text-level pre-filter that reads no files; R7 owns the final derivability decision — it MUST verify each claim against actual repo artifacts rather than relying on the candidate's self-description or 4d's estimate (see decision `2026-07-06-r7-sole-derivability-authority`). Because 4d no longer reads files, R7 is the one place artifact-verified derivability happens — skipping its artifact reads would remove that guarantee from the pipeline entirely.
...
- R7 is the sole authoritative artifact-verified derivability check (extract-candidates 4d is a cheap text pre-filter that reads no files) — R7 MUST verify claims against actual files, not just candidate text or 4d's estimate
...
3. Does R6 compare against existing vault entries (reusing extract-candidates Step 2 results in-memory, or CLI fallback when unavailable)?
